...
export class ListView extends React.Component<IListViewProps, {}> {
  render() {
    ...
    let noData = (
      <div className="sr-nodata-layout">
        <div className="sr-nodata"></div>
        <div className="sr-nodata-text">{T("抱歉，暂无数据")}</div>
      </div>
    );
    return (
    ...
    if (this.props.selection) {
      headArray.push(
        <th className="checkbox">
          <label>
            <input type="checkbox" onChange={this.onSelectAllRows} />
            <span>{T("全选")}</span>
          </label>
        </th>
      );
  ...
  private renderRow = (item: ICell[], index: number): JSX.Element => {
    ...
    return (
      <tr data-index={index} id={index.toString()}>
        {tdArray}
      </tr>
    );
  };
  ...
  private onSelectAllRows = (): void => {
    ...
    for (let i = 0; i < eles.length; i++) {
      (eles[i].firstChild.firstChild as HTMLInputElement).checked = (elesAll[0]
        .firstChild.firstChild as HTMLInputElement).checked;
    }
    this.onChangeSelect();
  };
  ...
  private onChangeSelect = (): void => {
    if (this.props.onSelected && typeof this.props.onSelected === "function") {
      ...
      for (let i = 0; i < eles.length; i++) {
        if ((eles[i].firstChild.firstChild as HTMLInputElement).checked) {
          let num = parseInt(
            (eles[i].parentNode as HTMLElement).getAttribute("data-index")
          );
          selectRows.push(num);
        }
      }
      this.props.onSelected(
        selectRows,
        (elesAll[0].firstChild.firstChild as HTMLInputElement).checked,
        this.onClearSelect
      );
    }
  };
