...
export class IconLink extends React.Component<IIconLinkProps, IIconLinkState> {
  ...
  render() {
    let maintenanceItem: JSX.Element[] = this.renderItem(
      this.props.items ? this.props.items : this.state.items
    );
    return <div className="sr-icon-link">{maintenanceItem}</div>;
  }
  ...
  private renderIconType = (data): JSX.Element => {
    switch (data.Shape) {
      case "solidCircle": {
        return (
          <div
            className={"sr-icon-box sr-square sr-circle"}
      ...
      case "hollowCircle": {
        return (
          <div
            className={"sr-icon-box sr-hollow sr-circle"}
      ...
      case "solidSquare": {
        return (
          <div
            className={"sr-icon-box sr-square"}
            style={{ background: data.Color }}
          >
            {this.renderIcon(data)}
          </div>
      ...
      case "hollowSquare": {
        return (
          <div
            className={"sr-icon-box sr-hollow"}
            style={{ borderColor: data.Color, color: data.Color }}
          >
            {this.renderIcon(data)}
          </div>
  ...
  private renderIcon = (data): JSX.Element => {
    if (data.FabricIcon) {
      return (
        <Icon iconName={data.FabricIcon} className="ms-IconExample sr-icon" />
      );
