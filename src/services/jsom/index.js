...
const funList = ["getCurrentUser", "getSiteUserInfo", "getListItem", "getListItemById", "updateListItemById",
  ...
  "isUserMemberOfGroup", "getUsersOfGroup","getCurrentUsersGroups", "updateDocumentLibraryItemById", "overWriteFile",
  "createDocumentLibraryItem", "createFolder", "checkSitePermission", "checkListPermission",
  "renderListData", "getListGUID", "getFileById","getListType","deleteVersionById","deleteVersionByCaml","deleteVersionByIdList"
];
...
getCurrentUsersGroups() {
  ...
  function getUserGroups(res, rej) {
    let context = info.context;
    let web = info.web;
    let user = web.get_currentUser();
    let group = user.get_groups();

    context.load(group);
    context.executeQueryAsync(onSuccess, JSOM.onError(rej));

    function onSuccess(sender, args) {
      let groupsEnumerator = group.getEnumerator();
      ...
      let result = new ResultMessage(true, groups);
      res(result);
    }
  }
  return new Promise(getUserGroups);
}
